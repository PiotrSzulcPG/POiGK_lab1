	...
	void Draw() const {
		if (type == WeaponType::BULLET) { // BULLET
			...
			if (gScore < 100) {
				...
				Rectangle lr = { transform.position.x - 2.f, transform.position.y - LASER_LENGTH, 4.f, LASER_LENGTH };
				DrawRectangleRec(lr, RED);
				Rectangle lrL = { transform.position.x - 2.f - 60, transform.position.y + 20 - LASER_LENGTH, 4.f, LASER_LENGTH };
				DrawRectangleRec(lrL, RED);
				Rectangle lrR = { transform.position.x - 2.f + 60, transform.position.y + 20 - LASER_LENGTH, 4.f, LASER_LENGTH };
				DrawRectangleRec(lrR, RED);
			}
		}
	}
	Vector2 GetPosition(int bulletPos) const {
		Vector2 transform_position = transform.position;
		if (bulletPos == -1) {
			transform_position.x -= 60;
			transform_position.y += 20;
		}
		else if (bulletPos == 0) {
	...
	float speed)
{
	...
	if (wt == WeaponType::LASER) {
		...
		//std::cout << "Pozycja myszki: " << GetMouseX() << "; " << GetMouseY() << "\n";
...
public:
	...
	void Draw() const override {
		if (!alive && fmodf(GetTime(), 0.4f) > 0.2f)
		{
			DrawText("GAME OVER", 350, 200, 50, WHITE);
			DrawText("Press 'R' to Restart", 370, 250, 20, RED);
			return;
		}
		Vector2 dstPos = {
...
public:
	...
	void Run() {
		...
		float weaponUpgradeTimer = 0.f;
		bool weaponUpgradedShown = false;
		...
		while (!WindowShouldClose()) {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					float dist = Vector2Distance((*pit).GetPosition(0), (*ait)->GetPosition());
					float distL = Vector2Distance((*pit).GetPosition(-1), (*ait)->GetPosition());
					float distR = Vector2Distance((*pit).GetPosition(1), (*ait)->GetPosition());
					...
					if (player->GetScore() > 100) {
						if (distL < (*pit).GetRadius() + (*ait)->GetRadius() || distR < (*pit).GetRadius() + (*ait)->GetRadius()) {
							ait = asteroids.erase(ait);
							pit = projectiles.erase(pit);
							player->UpdateScore();
							gScore = player->GetScore();
			...
			if (player->GetScore() >= 100 && !weaponUpgradedShown) {
				weaponUpgradeTimer = 3.f;
				weaponUpgradedShown = true;
			}
			if (weaponUpgradeTimer > 0.f) {
				weaponUpgradeTimer -= dt;
				if (weaponUpgradeTimer < 0.f) weaponUpgradeTimer = 0.f;
			}
			...
			// Render everything
			{
				...
				if (weaponUpgradeTimer > 0.f) {
					DrawText("Weapon Upgraded", 10, 70, 20, YELLOW);
				} else {
